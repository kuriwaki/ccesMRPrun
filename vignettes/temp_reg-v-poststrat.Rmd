...
vignette: >
  %\VignetteIndexEntry{Post-stratification and Aggregate Regression Adjustment in MRP}
...
Variables like party ID and religion are not available in the population target. One option is to impute this to the population target through a machine learning model trained on individual data. 

Another option that researchers often use is to include a area-level proportion and simply "control" for that aggregate variable in the regression. This is _not_ the same as poststratification, but we believe that it helps to account for this variable in some way. 
...
val <- acs_GA %>%
  ...
  summarize(prop_HS_truth = sum(is_HS*n)/sum(n),
            prop_BA_truth = sum(is_BA*n)/sum(n))
...
cces_df <- cces_GA %>%
  ...
  left_join(distinct(acs_df, cd, prop_HS_truth, prop_BA_truth), by = "cd") %>%
  ungroup()
```
...
scatter_45(mrp_plot,
           ...
           by_labels = c(omitted = "~ (1|cd)",
                         ...
                         partisan_mean = "(1|cd) + prop_clinton_true",
                         all_means = "(1|cd) + prop_HS_true + prop_clinton_true",
                         oracle = "~ (1|cd) + (1|education)"),
           xlab = "True Proportion of Adults with High School - only Education",
           ylab = "MRP Estimate") +
  labs(title = "Controlling for the aggregate proportion helps, but it's not poststratification",
       caption = "\nNote: Estimand is proportion of CD that has a high school education or less.
