...
Instead of including the aggregate information, others might opt to model a synthetic joint distribution. We provide a simple set of functions to implement this. We extend the ACS table assisted by a survey model using a function called [`cceMRPprep::synth_mlogit()`](https://www.shirokuriwaki.com/ccesMRPprep/reference/synth_mlogit.html). 
...
library(scales)
left_join(acs_synth,
          count(acs_df, cd, female, age, educ, wt = count, name = "count"),
...
The plot does not look great. It is after all perhaps not surprising that it is hard to estimate education from age bins and gender.
