...
fit <- fit_brms_binomial("yes | trials(n_response) ~ (1 | age) + (1 | educ) + (1 | cd)",
                cces_georgia)
```
...
fit <- read_rds("data-analysis/fit-brms_GA.rds")
summary(fit)
```
...
```{r}
poststrat_GA <- read_rds("data-analysis/by-cd-cell_poststrat_2018.rds")

drw <- poststrat_draws(fit, 
                       poststrat_tgt = poststrat,
                       orig_data = cces_georgia)
